...
class DeclarationCompiler {
	...
	public compile(config: Configuration, declarations: any[][]) {
		var css = a.flatten(declarations.map(declaration => {
			return this.compileDeclaration(config, declaration);
		})).join(config.declarationSeparator);
		return css.length ? config.oneIndent + css + config.newline : '';
	}

	private compileDeclaration(config: Configuration, declaration: any[]) {
